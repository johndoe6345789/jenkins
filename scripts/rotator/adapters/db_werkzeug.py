"""DB-werkzeug adapter: UPDATE a werkzeug-hashed password in a SQLite database
reached via `docker exec <container> python3 -c ...`.

Uses werkzeug.security.generate_password_hash inside the target container so
the hash format matches whatever version the app ships (scrypt by default in
werkzeug >= 2.0).

params keys:
    db_container     (str)  container name where the Flask app runs
    db_path          (str)  absolute path to SQLite file inside the container
    table            (str)  e.g. 'user_auth'
...
def rotate(params: dict, new_password: str) -> None:
    ...
    code = (
        ...
        f"h = generate_password_hash({new_password!r}); "
